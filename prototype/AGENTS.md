# AGENTS.md — Vespera Design System & Admin Template
...
Two linked deliverables built on one design system called **Vespera**:

| File | Role |
|---|---|
| `Vespera UI.html` | The **design-system showcase + documentation** site (copy-in component library). |
| `Vespera Admin.html` | The flagship **admin/operational template** (analytics console) built from Vespera. |
...
Vespera is a **copy-in library** — no package, no runtime. Each `.jsx` file registers its components onto `window` via `Object.assign(window, {...})` at the end. You include only the files whose components you use. Two CSS files are always required.

| File | Provides | Depends on |
|---|---|---|
| `styles.css` | **Design tokens**, theme blocks (`[data-theme]`), density/corners, base, **app-shell** (`.vsp-*` sidebar/topbar), `.btn`/`.card`/`.badge`/`.chip`, animated glow background. | — (required) |
...
| `vespera-ui.jsx` | **Core primitives:** `cx, Button (has loading), IconButton, Field, Input, Textarea, Select(base — overridden), InputAffix, Checkbox, Radio, RadioGroup, Switch, Slider, Badge, Tag, Kbd, Divider, Alert, Progress, Skeleton, Tabs, Tooltip, Breadcrumb, Pagination, Stepper, CardHead`. | icons |
| `vespera-overlays.jsx` | `Dialog, Sheet, Anchored, DropdownMenu, Popover, ToastHost, CommandPalette, useCmdK, useEsc`. Defines `agPortal()`. | vespera-ui |
| `vespera-select.jsx` | `Select` (**themed, overrides the native one in vespera-ui**), `Combobox, MultiSelect, TokenInput`. Defines `SelPanel`, `ComboList`. | overlays |
| `vespera-datepicker.jsx` | `Calendar, DatePicker, DateRangePicker`. | icons |
| `vespera-extras.jsx` | `Spinner, CircularProgress, Accordion, FileDropzone, EmptyState, Banner, OTPInput, CopyButton, AvatarGroup, NumberStepper, Tree, Timeline, DescriptionList, InlineEdit, Stat, SettingRow, VerticalTabs, NavItem, NavGroup`. | vespera-ui |
| `data.jsx` | Mock datasets (`CUSTOMERS, STATUSES, PLANS, revenue30, channelDonut, ...`). | — |
| `vespera-blocks.jsx` | Pro **operational blocks**: `OrdersBlock, TeamRolesBlock, SystemStatusBlock, ApiKeysBlock, AuditLogBlock, KanbanBlock`, `Block`. | all above |
| `vespera-forms.jsx` | Pro **form blocks**: `OnboardingWizard, AccountForm, BillingForm, FilterPanelBlock, AuthCardBlock`. | all above |
...
- **`.vsp-root` is the theming root.** Tokens live on `.vsp-root[data-theme=...]`. The app root `<div className="vsp-root" data-theme data-density data-corners data-dxside data-bganim style="--accent;--font-sans;--bg-accent">`.
- **Overlays MUST portal into `.vsp-root`, not `document.body`** — otherwise they lose all CSS variables (transparent bg / black text). `agPortal = () => document.querySelector('.vsp-root') || document.body`. This applies to Dialog, Sheet, DropdownMenu/Popover (Anchored), Toast, CommandPalette, Tooltip, SelPanel, DatePicker panel.
- **Use the themed `Select`, never a raw `<select>`.** `vespera-select.jsx` overrides `window.Select` with a themed dropdown that is **API-compatible** with native: `options`, `value`/`defaultValue`, `onChange` is called with an **event-like `{ target: { value } }`**, optional `searchable` (auto-on at ≥8 options). Native selects also get `color-scheme` from theme as a fallback.
...
## 4. Theming system (all via CSS vars on `.vsp-root`)
...
| `--font-sans` (inline) | font stack | typeface (Jakarta/Grotesk/Sora options). `.vsp-root` applies it so descendants inherit. |
...
| `data-bganim` + `--bg-accent` | `on/off` + hex | animated blurred glow orbs (`.vsp-bg`), color-pickable. |
...
## 5. The showcase (`Vespera UI.html`)
...
## 6. The admin template (`Vespera Admin.html`)
...
1. Edit files. Reuse tokens + existing `.ui-*`/`.vsp-*` classes; add new `Object.assign(window,{...})` exports for new components.
...
5. When adding a library component: add CSS to `ui.css`, component to the right `vespera-*.jsx`, export to `window`, showcase it in the matching showcase section, AND add a `SC_INDEX` entry + a `showcase-docs.jsx` API entry.
