...
const DOC_FILES = [
  ...
  ['vespera-ui.jsx', 'Button · Input · Field · Checkbox · Radio · Switch · Slider · Badge · Tag · Tabs · Tooltip · Alert · Progress · Skeleton · Breadcrumb · Pagination · Stepper', 'Core'],
  ['vespera-overlays.jsx', 'Dialog · Sheet · Popover · DropdownMenu · Toast · CommandPalette', 'needs vespera-ui'],
  ['vespera-select.jsx', 'Select (themed) · Combobox · MultiSelect · TokenInput', 'needs overlays'],
  ['vespera-datepicker.jsx', 'Calendar · DatePicker · DateRangePicker', 'standalone'],
  ['vespera-extras.jsx', 'Spinner · CircularProgress · Accordion · FileDropzone · EmptyState · Banner · OTPInput · CopyButton · AvatarGroup · NumberStepper · Tree · Timeline · DescriptionList · InlineEdit · Stat · SettingRow · VerticalTabs · NavGroup', 'needs vespera-ui'],
  ['components.jsx', 'AreaChart · BarChart · Donut · Sparkline · StatCard · Avatar', 'standalone'],
  ['vespera-blocks.jsx, vespera-forms.jsx', 'Pro composed blocks (tables, kanban, wizards, billing…)', 'needs all above'],
];

const DOC_API = [
  { name: 'Button', file: 'vespera-ui.jsx', desc: 'Action trigger with five variants, two sizes, optional icons and a loading state.',
...
<Button variant="ghost" loading>Saving…</Button>` },
  { name: 'Input / Field', file: 'vespera-ui.jsx', desc: 'Text input, wrapped by Field for a label, hint and error message.',
  ...
  { name: 'Select', file: 'vespera-select.jsx', desc: 'Themed dropdown — a drop-in replacement for native <select>. Auto-enables search for 8+ options.',
...
/>` },
  { name: 'Combobox / MultiSelect', file: 'vespera-select.jsx', desc: 'Searchable single / multi select with type-ahead and keyboard nav.',
...
/>` },
  { name: 'DatePicker', file: 'vespera-datepicker.jsx', desc: 'Calendar popover for a single date. DateRangePicker takes { start, end }.',
...
<DateRangePicker value={range} onChange={setRange} />` },
  { name: 'Dialog / Sheet', file: 'vespera-overlays.jsx', desc: 'Centered modal (Dialog) or right drawer (Sheet). Portalled, Esc-dismissable, theme-aware.',
  ...
  { name: 'Toast', file: 'vespera-overlays.jsx', desc: 'Global notifications. Mount <ToastHost/> once, then call window.toast() anywhere.',
...
window.toast({ tone: 'pos', title: 'Saved', body: 'Changes are live.' })` },
  { name: 'DropdownMenu', file: 'vespera-overlays.jsx', desc: 'Anchored action menu with headings, separators, icons, shortcuts and danger items.',
  ...
  { name: 'Tabs / Accordion', file: 'vespera-ui.jsx · vespera-extras.jsx', desc: 'Underline Tabs and an animated, single/multi Accordion.',
...
<Accordion defaultOpen={[0]} items={[
  ...
  { name: 'Badge', file: 'vespera-ui.jsx', desc: 'Status pill with semantic tones and an optional dot.',
...
<script type="text/babel" src="vespera-ui.jsx"></script>      <!-- Button, Input, Badge… -->
<script type="text/babel" src="vespera-overlays.jsx"></script> <!-- Dialog, Toast… -->
<script type="text/babel" src="vespera-select.jsx"></script>   <!-- Select, Combobox -->`} />
      </div>
      <div>
        <h3 className="doc-h3">3 · Use it</h3>
        <p className="doc-p" style={{ margin: '6px 0 12px' }}>Wrap your app in an element with <span className="doc-pill">.vsp-root</span> and a theme attribute — that's what scopes the tokens (and overlays portal into it).</p>
...
icons.jsx · vespera-ui.jsx      (Field, Input, Button, Checkbox…)
vespera-datepicker.jsx          (DatePicker)`} />
      </div>
...
function DocTheming() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 760 }}>
      <p className="doc-p">Everything is driven by CSS custom properties on <span className="doc-pill">.vsp-root</span>. Re-theme the whole system by overriding a handful of variables — no component edits.</p>
      <div>
        ...
        <DocCode code={`<div class="vsp-root"
...
const RELEASES = [
  ...
  { v: '1.3.0', date: 'Jun 9, 2026', items: [['Added', 'Spinner, CircularProgress, Accordion, FileDropzone, EmptyState, Banner, OTP, NumberStepper, Tree, Timeline, DescriptionList, InlineEdit, DatePicker; Button loading.'], ['Fixed', 'Overlays portal into .vsp-root (theme-aware popups); accent caret.']] },
