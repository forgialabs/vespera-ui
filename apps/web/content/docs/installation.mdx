...
See the [React components API](/docs/reference/react) for every component and its props.
...
way. See the [Frameworks guide](/docs/frameworks) for install and usage in each.
...
That's it — see [Quick start](/docs/quick-start) to build something, or
[Theming](/docs/theming) to make it yours.
