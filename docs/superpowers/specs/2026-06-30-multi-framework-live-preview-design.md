...
- A standalone cross-framework E2E harness separate from Storybook (the test-runner on
  stories covers it).
- Rewriting framework wrappers for parity (we *surface* gaps; closing them is follow-on work).
...
- **Unifies the two goals into one source of truth**: a *story* is the live demo **and** the
...
- The selector becomes **parity-aware** — a framework with no story for a component renders
  disabled ("not yet available"), so gaps show up *in the UI itself*.
